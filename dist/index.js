...
export default function herdrAutoReload(pi) {
    ...
    pi.registerCommand(COMMAND_NAME, {
        ...
        handler: async (_args, ctx) => {
            ...
            const deliveries = await Promise.allSettled(remoteAgents.map(async (agent) => {
                const result = await pi.exec("herdr", ["agent", "prompt", agent.pane_id, RELOAD_COMMAND], { timeout: 5_000 });
                if (result.code !== 0)
                    throw new Error(result.stderr || result.stdout || "Herdr rejected the reload request.");
            ...
            ctx.ui.notify(`Requested reload for ${delivered} remote Pi session${delivered === 1 ? "" : "s"}${failed ? `; ${failed} failed` : ""}. Reloading this session.`, failed ? "warning" : "info");
